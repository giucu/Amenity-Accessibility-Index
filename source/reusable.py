...
def create_hex_grid(gdf, radius_meters):
    """ Using a predefined geoDataFrame, returns one with hexagonal grid cells overlayed
    """
...
def assign_population_to_grid(pop_raster_path, grid, plot=True):
    """
    Assign population to a hex grid using proportional area-based allocation, returned updated gdf
    """
    # safely handle mixed geometry types without index conflicts
    grid_out = grid.copy()
    grid_out = grid_out.explode(index_parts=False)       
    grid_out["geometry"] = grid_out["geometry"].buffer(0) # fix invalid geoms
    grid_out = grid_out[~grid_out.geometry.is_empty]     
    grid_out = grid_out.reset_index(drop=True)            
...
def create_origins(grid, grid_pop=None):
    """
    Combines 2 gdf of the hex grids + population into 1 gdf of population associated at geographic midpoints of grid cells
    """
...
def create_POIs(osm, summary=False):
    """Filter and recategorise POI gdf
    """
...
def suggested_beta(cutoff_minutes, target_weight=0.5):
    """
    Returns beta for decay to retains target_weight of its value after cutoff_minutes .
    """
    return -np.log(target_weight) / cutoff_minutes

def compute_gravity_scores(travel_time_matrix, destinations, beta=0.08):
    """apply decay onto ttm, returns updated df
    """
...
def min_travel_time(travel_time_matrix, destinations):
    """returns a df that has computed minimum travel times to reach each opportunity category
    """
...
def weight_pop(results, origins, pop_col="population", score_cols=None, normalisation=100, return_full=False):
    """
    Updated results df by merging results and origins based on geometry (normalisation : score per X inhabitants)
    """
...
def compute_diversity_scores(results, count_cols, id_col="from_id"):
    """
    Computes amenity diversity scores per hex
    """
...
def merge_geometry(results, grid, from_id_col="from_id"):
    """adds geometric coordinated onto some df based on cell id """
...
def plot_hex_score(results_geo, score_col, from_id_col="from_id", scheme="Quantiles",
    ...
    if n_missing > 0:
        print(f"Note: {n_missing} hexes have NaN for '{score_col}'")
...
def build_city_geojson(grid, grid_pop, osm, tn, output_path=None, transport_modes=None, max_time=30, beta=0.08, gravity_normalisation=100,):
    """
    Runs complete accessibility metrics pipeline on given grid + osm + travel network
    ...
    Returns fully scored gdf with all metrics attached
    """
    ...
    counts_raw = compute_gravity_scores(ttm.copy(), destinations, beta=0) # basic counts
    ...
    pop_weighted = weight_pop(gravity, origins, score_cols=gravity_cols, normalisation=gravity_normalisation, return_full=False,)
    ...
    # merge all score dataframes so far
    ...
    # apply EPSG:4326 for easier mapping application later
